...
if ($id > 0) {
    $stmt = getDb()->prepare('SELECT * FROM board_members WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch();
    if ($row) {
        $name = $row['name'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_image']) && $id > 0) {
    $stmt = getDb()->prepare('UPDATE board_members SET image = ? WHERE id = ?');
    $stmt->execute(['', $id]);
    header('Location: /admin/board-edit.php?id=' . $id);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($error === '') {
        if ($id > 0) {
            $stmt = getDb()->prepare(
                'UPDATE board_members SET name = ?, role = ?, bio = ?, image = ?, sort_order = ? WHERE id = ?'
            );
            $stmt->execute([$name, $role, $bio, $image, $sortOrder, $id]);
        } else {
            $stmt = getDb()->prepare(
                'INSERT INTO board_members (name, role, bio, image, sort_order) VALUES (?, ?, ?, ?, ?)'
            );
            $stmt->execute([$name, $role, $bio, $image, $sortOrder]);
        }
...
<head>
  ...
  <title><?php echo $id > 0 ? 'Vorstandsmitglied bearbeiten' : 'Neues Vorstandsmitglied'; ?> &ndash; Naturschutzsp&uuml;rhunde Admin</title>
...
<body>
  ...
  <div class="panel">
    <h1><?php echo $id > 0 ? 'Vorstandsmitglied bearbeiten' : 'Neues Vorstandsmitglied'; ?></h1>
    <?php if ($error !== ''): ?>
    ...
    <form method="post" enctype="multipart/form-data">
      ...
      <input type="text" id="role" name="role" value="<?php echo htmlspecialchars($role); ?>" placeholder="z.B. Präsidentin">

      <label for="bio">Kurzbeschrieb</label>
      <textarea id="bio" name="bio"><?php echo htmlspecialchars($bio); ?></textarea>

      <label for="image">Foto</label>
      <?php if ($image !== ''): ?>
        <img class="current-image" src="/uploads/<?php echo htmlspecialchars($image); ?>" alt="Aktuelles Foto">
      <?php endif; ?>
